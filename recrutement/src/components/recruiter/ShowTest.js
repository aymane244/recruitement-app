import axios from "axios";
import React, { useContext, useEffect, useState } from "react";
import { Navigate, useLocation, useNavigate } from "react-router-dom";
import { Auth } from "../context/AuthContext";

export default function ShowTest(){
    const navigate = useNavigate()
    const [isLoading, setIsLoading] = useState(true)
    const location = useLocation();
    const { recruiter, api, tokenRe  } = useContext(Auth)
    const [quizes, setQuiz] = useState([]);
    useEffect(() => {
        async function getQuizes() {
            const quiz = await api.get('/api/show-tests');
            setQuiz(quiz.data.quizes)
            setIsLoading(false)
        }
        getQuizes()
    }, [])
    function deletes(id, name){
        const conf = window.confirm("Voulez vous supprimé l'annonce "+name);
        if(conf){
            axios.delete("http://127.0.0.1:8000/api/delete-test/" + id).then((response) => {
                navigate('/recruiter/tests/', { state: response.data.message })
                // navigate(0)
            })
        }
    }
    let i = 1;
    return(
        <div className="container mt-5">
            {tokenRe ?
                <>
                    {isLoading === true ? <div className='text-center margin-img'><img src='/images/loading-gif.gif' alt='loading'/></div> :
                    <div>
                        {location.state && <div className="alert alert-success mt-3 w-50 mx-auto text-center" role="alert">{location.state}</div>}
                        <div className="text-center my-4">
                            <a href="add-test" className="btn btn-primary fs-4" target="_blank">Créer test</a>
                        </div>
                        {quizes.length > 0 ?
                        <table className="table table-dark">
                            <thead className="text-center">
                                <tr>
                                    <th colSpan={6}> {recruiter.name}</th>
                                </tr>   
                                <tr>
                                    <th>#</th>
                                    <th>Annonce</th>
                                    <th>Titre de test</th>
                                    <th>Action</th>
                                </tr>
                            </thead>
                            <tbody className="text-center">
                                {quizes.map((list, index) => {
                                    return (
                                        <tr key={index}>
                                            <th> {i++} </th>
                                            <th> {list.job_position} </th>
                                            <th> {list.titre} </th>
                                            <th>
                                                <div className="d-flex justify-content-around align-items-center">
                                                    <div>
                                                        <a href={"/recruiter/test-results/"+list.quize_id} className="btn btn-primary">Voir les résultats</a>
                                                    </div>
                                                    <div>
                                                        <a href={"/recruiter/edit-test/"+list.quize_id} target="_blank"><i className="fa-solid fa-pen-to-square text-success fs-4"></i></a>
                                                    </div>
                                                    <div>
                                                        <button className="bg-transparent border-0"onClick={()=>deletes(list.id, list.titre)}>
                                                            <i className="fa-solid fa-trash text-danger fs-4"></i>
                                                        </button>
                                                    </div>
                                                </div>
                                            </th>
                                        </tr>
                                    )
                                })}
                            </tbody>
                        </table> : <h2 className="text-center mt-5 pt-5">Pas de QCM pour le moment</h2>
                        }   
                    </div>}
                </> 
                : <Navigate to="/login" />
            }
        </div>
    )
}